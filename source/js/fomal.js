...
function showWelcome() {
  ...
  let dist = getDistance(104.0052716, 30.6591334, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
...
function getWeibo() {
  fetch('https://weibo.love.sc.cn/api').then(data => data.json()).then(data => {  // 这里要写上你的API!!!
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("08/12/2023 19:01:34");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='/assets/BestBlog-上班中💻.svg' title='什么时候能够实现财富自由呀~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='/assets/BestBlog-下班休息啦🌙.svg' title='下班了就该开开心心地玩耍~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://blog.love.sc.cn/importance/pc%E7%AB%AF%E7%99%BD%E5%A4%A9.jpg);
    --darkmode-bg:url(https://blog.love.sc.cn/importance/PC%E7%AB%AF%E9%BB%91%E5%A4%9C.JPG);
